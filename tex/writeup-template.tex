...
\section*{Task 2 - Is there a need to find edge constraints when k $\geq$ 1}
    ...
    This is an unrealistic assumption and even if execution is not an issue, planning a path like this would mean robots will be too close to each other.
...
The sum of the lengths of the paths of each agent is smaller than TA-RANDOM and because TA-DISTANCE uses the hungarian algorithm which guarantees the
...
\begin{table}
    ...
    In this case, the difference between TA-DISTANCE and TA-CBS is negligible but in cases when the limitations of the hungarian algorithm are encountered,
...
\section*{Task 15}
    ...
    This means that for a staircase (zigzag) path of $k$ moves alternating between two directions, the holonomic robot takes $k$ actions while the SMART robot requires $k$ moves plus $k-1$ rotations, totalling $2k-1$ actions --- nearly twice as many, or equivalently, the holonomic robot has approximately half the cost.
    ...
    """
        ...
        while len(open_list) > 0:
            ...
            if curr['loc'] == goal_loc:
                ...
                if can_stop:
                    return get_path(curr), curr["g_val"]  (*@\textcolor{red}{\textbf{<-- MODIFIED}}@*)
            ...
            for dir in range(len(directions)):
                ...
                child["g_val"] = increase_cost_2(child)  (*@\textcolor{red}{\textbf{<-- MODIFIED}}@*)
                #############################################
...
def increase_cost_2(node: Dict) -> int:
    ...
    if node['g_val'] > 1:
        ...
        p2p_action = (prev_prev_loc[0] - prev_loc[0], prev_prev_loc[1] - prev_loc[1])  (*@\textcolor{red}{\textbf{<-- MODIFIED}}@*)
        p2c_action = (prev_loc[0] - curr_loc[0], prev_loc[1] - curr_loc[1])            (*@\textcolor{red}{\textbf{<-- MODIFIED}}@*)
...
\begin{table}
    ...
    \begin{tabular}{lll}
        ...
        \midrule
        Exp0, k1 - TA-CBS & 7, 2/2 in 0s &  \\
        Exp0 - SMART &  &  \\
        Exp2\_2 - TA-CBS & 7, 3/5 in 0.0s &  \\
        Exp2\_2 - SMART &  &  \\
        Test 20, k1 - TA-CBS & 33, 358/666 in 2.2s & 40, 358/666 in 2.7s\\
        Test 20 - SMART & & 13, MS=23.5s, Avg=15.9  \\
        Test 21, k1 - TA-CBS & 57, 793/1442 in 8.3s & 70, 793/1442 in 10.1s \\
        Test 21 - SMART & & 28, MS=21.3s, Avg=16.2s  \\
        Test 22, k1 - TA-CBS & 47, 18/35 in 0.1s &  \\
        Test 22 - SMART & 10, MS=25.3s, Avg=17.9s &  \\
        \bottomrule
    \end{tabular}
    \caption{}
    \label{tab:}
